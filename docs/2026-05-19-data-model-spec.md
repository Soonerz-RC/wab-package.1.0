...
Front-end filters use `status_category`. Detail pages display `status_raw` so the regulatory context (which reg case) is not lost.
...
ORRI rows lack a "deal" field but have duplicates on the same STR with different decimals and dates. The `row_hash` is computed as a SHA-1 hex digest (first 8 chars) of `county|str|nra|dol|exp` — that uniquely identifies an ORRI grant within the Inventory and lets the registry find the same grant across refreshes.
...
| `status_raw` | string \| null | Verbatim from Inventory |
| `status_category` | string | Normalized: `LEASED|HBP|OPEN|PENDING|OTHER` |
